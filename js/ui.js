...
(function () {
  ...
  /* ---------- Iconify glyphs ----------------------------------------
     Icon set bundled in vendor/icons.js (generated by build/icons.mjs),
     so the demo still runs offline. The glyph inherits the text colour,
     which keeps every pictogram on the brand palette.
     ------------------------------------------------------------------ */
  function Icon({ name, className, title }) {
    const ico = (globalThis.SALUS_ICONS || {})[name];
    if (!ico) return null;
    return h("svg", {
      viewBox: ico.vb,
  ...
  /* Framed pictogram standing in for a photo (questions, situations,
     rooms, profiles). `className` sizes the frame, `size` the glyph. */
  function IconTile({ name, className, size, tone, title }) {
    return h("div", {
      className: cx("flex items-center justify-center bg-salus-cyan/5 border border-salus-cyan/25", className || "w-full h-24 rounded-lg")
    }, h(Icon, { name, title, className: cx(size || "w-8 h-8", tone || "text-salus-cyan") }));
  }
  ...
  function ChoiceCard({ label, hint, img, icon, selected, onClick, badge, disabled, reason }) {
    return h("button", {
      onClick, disabled,
      className: cx(
        "tile relative flex flex-col text-left rounded-2xl border-2 bg-white p-4 w-full",
      ...
      icon && h(IconTile, { name: icon, className: "w-full h-24 rounded-lg mb-3", size: "w-12 h-12" }),
      !icon && img && h("img", { src: img, alt: "", className: "w-full h-24 object-cover rounded-lg mb-3 bg-slate-100" }),
  ...
  window.UI = { h, cx, eur, Icon, IconTile, PriceTag, Btn, ChoiceCard, SectionTitle, Modal, VideoPlaceholder, StepBar, InfoBox, Stepper };
})();
